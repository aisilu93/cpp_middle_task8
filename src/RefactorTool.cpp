...
void RefactorHandler::handle_miss_override(const CXXMethodDecl *Method, DiagnosticsEngine &Diag, SourceManager &SM) {
    ...
    while (!invalid) {
        ...
        cur_symbol = SM.getCharacterData(loc, &invalid);
    }
    if (loc.isValid())
...
auto NvDtorMatcher() {
    ...
    return cxxRecordDecl(isDerivedFrom(cxxRecordDecl(
        isDefinition(), has(cxxDestructorDecl(unless(isImplicit()), unless(isVirtual())).bind("nonVirtualDtor")))));
}
...
auto NoOverrideMatcher() {
    ...
    return cxxMethodDecl(isOverride(), unless(cxxDestructorDecl()), unless(hasAttr(attr::Override)))
